...
/**
 * Run the analysis phase for an imported project:
 * ANALYZER agent reads the existing codebase and produces a project profile.
 */
export async function runAnalysis(
  ...
  const task: AgentTask = {
    ...
    description: `Analyze the imported codebase and produce a comprehensive project profile`,
    priority: 10,
  };
  ...
  try {
    await prisma.message.create({
      data: {
        ...
        role: "system",
        type: "text",
        content: `Analyzing imported project from ${project.sourceRepoUrl ?? "local source"}...`,
      },
    });
    ...
    const result = await orchestrator.spawnAgent(
      ...
      task,
      getProjectDir(projectId),
      `This is an imported project${project.sourceRepoUrl ? ` from ${project.sourceRepoUrl}` : ""}. Analyze the entire codebase and produce a comprehensive project profile.`
    );

    // Save the analysis as the project plan
    await prisma.project.update({
      where: { id: projectId },
